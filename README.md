...
If your Railway deployment fails health checks:

1. **Check Logs**: View Railway logs for startup errors
2. **Environment Variables**: Ensure all required environment variables are set
3. **Database Connection**: Verify DATABASE_URL is correct and accessible
4. **Port Configuration**: Ensure PORT environment variable is set (Railway sets this automatically)
5. **Startup Time**: The application includes a 5-second startup delay to ensure services are ready

### Common Issues

- **Configuration Validation Failed**: Check that all required environment variables are set
- **Database Connection Error**: Verify DATABASE_URL format and database accessibility
- **Bot Token Invalid**: Ensure BOT_TOKEN is correct and bot is not blocked
- **Port Already in Use**: Railway automatically assigns ports, don't override PORT variable

### Local Testing

Test the health check locally before deploying:
...
- **Bot Status**: Whether the Telegram bot is running and connected
- **Database Status**: Database connection and responsiveness
- **Blockchain Monitor**: Status of the blockchain monitoring service
- **Overall Health**: Combined status of all critical services

The health check is automatically configured for Railway deployment and will retry for up to 1m40s if the service is unavailable during startup.
